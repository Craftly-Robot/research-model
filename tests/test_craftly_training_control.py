...
from src.craftly.evaluation import benchmark_suites as benchmark_suites_module
from src.craftly.learning import ablation_runner as ablation_runner_module
from src.craftly.evaluation.benchmark_suites import BenchmarkSuitesReport
from src.craftly.evaluation.eval_runner import EvalRunReport, aggregate_scores, evaluate_checkpoint_with_schedule, regression_flags
from src.craftly.learning.mixer import build_mix
...
from src.craftly.model_ops.pretrain_loop import build_learning_rate_scheduler, save_training_checkpoint
from src.craftly.model_ops.tokenizer_pipeline import SPECIAL_TOKENS, TokenizerTrainConfig, train_bpe_tokenizer
from src.craftly.model_ops.torch_decoder import CraftlyDecoderLM, tiny_smoke_config
...
@unittest.skipIf(torch is None, "torch is required for Craftly training-control tests")
class CraftlyTrainingControlTest(unittest.TestCase):
    def test_scientific_experiment_authority_is_default_cli_help(self) -> None:
        ...
        self.assertIn("Craftly scientific experiment authority", output.getvalue())
    ...
    def scientific_campaign(self, experiment_type: str) -> ScientificExperimentCampaignContract:
        common = {
            ...
            "description": "A controlled scientific experiment used by the Craftly unit-test evidence authority.",
    ...
    def scientific_manifest(
        ...
        for index, name in enumerate(campaign.required_evaluation_suites):
            if name in {"CraftlyDefensiveSecurity", "CraftlyRepositoryScorecard"}:
    ...
    def test_scientific_evaluation_assembles_code_and_repository_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            repository_hash = manifest.evaluation_inputs[
                "suite:CraftlyRepositoryScorecard"
            ].sha256
            tasks = [
                {
