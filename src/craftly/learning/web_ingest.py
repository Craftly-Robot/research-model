...
class SourceSpec(StrictModel):
    ...
    trust_tier: Literal["quarantine", "reviewed", "trusted", "protected_holdout"] = (
        "quarantine"
    )
    ...
    @field_validator(
        "license_evidence_sha256", "legal_approval_sha256", "approval_request_sha256"
    )
    @classmethod
    ...
    def finalize_identity(self) -> "SourceSpec":
        ...
        if (
            self.trust_tier == "protected_holdout"
            and self.approved_use != "evaluation_only"
        ):
...
class CrawlConfig(StrictModel):
    user_agent: str = "CraftlyResearchBot/0.1 defensive AI dataset builder"
...
class WebCorpusIngestor:
    ...
    async def ingest(
        self, sources: list[SourceSpec], output_path: str | Path
    ) -> IngestReport:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest allowlisted defensive web corpus."
    )
