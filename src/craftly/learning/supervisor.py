...
class SupervisorConfig(StrictModel):
    ...
    heartbeat_path: str = "artifacts/craftly/supervisor/heartbeat.json"
    status_path: str = "artifacts/craftly/supervisor/status.json"
...
async def run_supervisor(config: SupervisorConfig) -> SupervisorReport:
    ...
    if readiness.status == "block":
        raise RuntimeError(
            f"data platform readiness blocked supervisor: {readiness.model_dump()}"
        )
    ...
    for cycle in range(1, config.max_cycles + 1):
        ...
        try:
            ...
            last_error = None
            fetched = sum(
                int(item.get("fetched", 0)) for item in worker_report.get("reports", [])
            )
            if fetched == 0:
                break
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run supervised Craftly data crawl cycles."
    )
    parser.add_argument(
        "--sources",
    ...
    parser.add_argument(
        "--object-store-uri",
        default=os.environ.get(
            "CRAFTLY_OBJECT_STORE_URI", "s3://craftly-datasets/pretraining"
        ),
    )
    parser.add_argument(
        "--heartbeat-path",
    ...
    parser.add_argument(
        "--status-path",
        default=os.environ.get(
            "CRAFTLY_STATUS_PATH", "artifacts/craftly/supervisor/status.json"
        ),
    )
    parser.add_argument(
        "--worker-replicas",
        type=int,
        default=int(os.environ.get("CRAFTLY_WORKER_REPLICAS", "1")),
    )
    parser.add_argument(
        "--async-workers",
        type=int,
...
def _config_from_args(args: argparse.Namespace) -> SupervisorConfig:
    if not args.postgres_dsn:
        raise ValueError("CRAFTLY_DATABASE_URL or --postgres-dsn is required")
