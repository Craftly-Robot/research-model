...
def _resource_name(doc: dict[str, Any]) -> str:
    return (
        f"{doc.get('kind', 'Unknown')}/{doc.get('metadata', {}).get('name', 'unnamed')}"
    )
...
def _container_issues(
    ...
    for container in pod_spec.get("containers", []):
        ...
        for env in container.get("env", []):
            ...
            is_secret_like = any(
                secret_word in name_key
                for secret_word in ("secret", "password", "token", "key")
            )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate Craftly Kubernetes manifests."
    )
    parser.add_argument(
        "--files",
        nargs="+",
