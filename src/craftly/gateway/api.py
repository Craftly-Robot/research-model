"""Consolidated Craftly Gateway API."""
...
from src.craftly.db import LocalStore, PostgresRAGStore, PostgresRAGStoreFactory
from src.craftly.evaluation.benchmarks import BenchmarkHarness, built_in_security_tasks
from src.craftly.identity import (
    AuthError,
    auth_status,
    create_jwt,
    install_auth,
...
from src.craftly.indexing import (
    ContextBuilder,
    RepositoryIndexer,
    VectorBackendConfig,
    create_vector_index,
...
from src.craftly.learning.versioning import DatasetLedger
from src.craftly.memory import MemoryIngestRequest, UnifiedMemoryManager
from src.craftly.model_ops.backends import active_model_health, list_model_profiles
from src.craftly.model_ops.foundation import PretrainingRunSpec, foundation_status
from src.craftly.observability import METRICS, install_observability
from src.craftly.patches import PatchPreviewRequest, PatchService, PatchVerifyRequest
...
from src.craftly.shared.schemas import CraftlyRunReport, CraftlyRunRequest
...
def resolve_registered_repo_path(value: str) -> Path:
    ...
    if normalized in {"", "."}:
        ...
        if any(
            part in {"", ".", ".."} or SAFE_REPO_PATH_PART.fullmatch(part) is None
            for part in parts
        ):
    ...
    configured = [
        item
        for item in os.environ.get("CRAFTLY_PROJECT_ROOTS", "").split(os.pathsep)
        if item
    ]
    roots = (
        [Path(item).expanduser().resolve() for item in configured]
        if configured
...
class VectorSearchRequest(BaseModel):
    ...
    backend: str = Field(
        default="local_hashing", pattern="^(local_hashing|faiss|hnsw|qdrant|pgvector)$"
    )
...
class SessionCreateRequest(BaseModel):
    project_id: str = Field(min_length=1)
    title: str = Field(default="Craftly Session", min_length=1, max_length=200)
...
def rag_store(organization_id: str) -> LocalStore | PostgresRAGStore:
    ...
    if os.environ.get("CRAFTLY_ENV", "development").lower() != "production":
        return STORE
    dsn = os.environ.get("CRAFTLY_DATABASE_URL", "")
    if not dsn:
        raise RuntimeError("production RAG requires CRAFTLY_DATABASE_URL")
    with RAG_STORE_FACTORY_LOCK:
        if RAG_STORE_FACTORY is None:
            RAG_STORE_FACTORY = PostgresRAGStoreFactory(
                dsn,
                min_pool_size=int(os.environ.get("CRAFTLY_RAG_DB_POOL_MIN", "1")),
                max_pool_size=int(os.environ.get("CRAFTLY_RAG_DB_POOL_MAX", "20")),
            )
            RAG_STORE_FACTORY_DSN = dsn
        elif dsn != RAG_STORE_FACTORY_DSN:
            raise RuntimeError(
                "CRAFTLY_DATABASE_URL changed after the shared RAG pool was initialized"
            )
        return RAG_STORE_FACTORY.for_organization(organization_id)


def require_project_access(
    request: Request, project_id: str, scope: str
) -> dict[str, Any]:
...
def production_vector_config() -> VectorBackendConfig:
    production = os.environ.get("CRAFTLY_ENV", "development").lower() == "production"
...
def _dataset_version_manifest_path(version_id: str) -> Path:
    ...
    root = (
        Path(os.environ.get("CRAFTLY_DATASET_VERSIONS_ROOT", "data/production"))
        .expanduser()
        .resolve()
    )
    ...
    if not candidate.is_file():
        ...
        if direct.is_file():
            payload = ProductionDatasetManifest.model_validate_json(
...
async def require_training_job_access(
    request: Request, job_id: str, scope: str
) -> TrainingJob:
    require_scope(request, scope)
    ...
    if (
        AUTH_CONFIG.enabled
        and "training:admin" not in request_scopes(request)
        and not job_bound
        and job.owner_id != request_owner(request)
    ):
...
async def issue_auth_token(request: AuthTokenRequest) -> dict[str, object]:
    if not AUTH_CONFIG.jwt_secret:
        raise HTTPException(status_code=503, detail="CRAFTLY_JWT_SECRET is required")
    try:
...
def training_access_token(account: ServiceAccount) -> str:
    if not AUTH_CONFIG.jwt_secret:
        raise HTTPException(
            status_code=503,
            detail="CRAFTLY_JWT_SECRET is required for workspace tokens",
        )
    return create_jwt(
...
async def create_training_service_account(
    request: TrainingServiceAccountRequest, http_request: Request
) -> dict[str, Any]:
    try:
        require_scope(http_request, "training:admin")
        credential = await TRAINING_WORKSPACE.create_service_account(
...
async def exchange_training_token(
    request: TrainingTokenExchangeRequest,
) -> dict[str, Any]:
    account = await TRAINING_WORKSPACE.authenticate_bootstrap(request.bootstrap_token)
    if account is None:
        raise HTTPException(
            status_code=401, detail="invalid workspace bootstrap credential"
        )
...
async def refresh_training_token(
    request: TrainingTokenRefreshRequest,
) -> dict[str, Any]:
    account = await TRAINING_WORKSPACE.authenticate_refresh(
        request.session_id, request.refresh_token
    )
    if account is None:
        raise HTTPException(
            status_code=401, detail="refresh session is invalid, expired, or revoked"
        )
    return {
        "token_type": "Bearer",  # nosec B105 - OAuth token type label, not a credential.
        "access_token": training_access_token(account),
...
async def revoke_training_token(
    request: TrainingTokenRefreshRequest, http_request: Request
) -> dict[str, Any]:
    try:
        require_scope(http_request, "training:jobs:read")
        revoked = await TRAINING_WORKSPACE.revoke_refresh(
            request.session_id, request.refresh_token
        )
        if not revoked:
...
async def create_training_job(
    request: TrainingJobCreateRequest, http_request: Request
) -> dict[str, Any]:
    try:
        require_scope(http_request, "training:jobs:create")
        job = await TRAINING_WORKSPACE.create_job(
            request, owner_id=request_owner(http_request)
        )
        METRICS.inc(
            "craftly_training_jobs_total",
...
async def list_training_jobs(http_request: Request, limit: int = 100) -> dict[str, Any]:
    try:
        ...
        owner = (
            None
            if "training:admin" in request_scopes(http_request)
            else request_owner(http_request)
        )
...
async def get_training_job(job_id: str, http_request: Request) -> dict[str, Any]:
    try:
        job = await require_training_job_access(
            http_request, job_id, "training:jobs:read"
        )
        return job.model_dump(mode="json")
    except PermissionError as exc:
        raise HTTPException(status_code=403, detail=str(exc)) from exc
...
async def cancel_training_job(job_id: str, http_request: Request) -> dict[str, Any]:
    try:
        ...
        job = await TRAINING_WORKSPACE.cancel_job(
            job_id, actor_id=request_owner(http_request)
        )
        METRICS.inc("craftly_training_job_transitions_total", target="cancelled")
        return job.model_dump(mode="json")
    except PermissionError as exc:
        raise HTTPException(status_code=403, detail=str(exc)) from exc
    except KeyError as exc:
...
async def resume_training_job(job_id: str, http_request: Request) -> dict[str, Any]:
    try:
        ...
        job = await TRAINING_WORKSPACE.resume_job(
            job_id, actor_id=request_owner(http_request)
        )
        METRICS.inc("craftly_training_job_transitions_total", target="queued")
        return job.model_dump(mode="json")
    except PermissionError as exc:
        raise HTTPException(status_code=403, detail=str(exc)) from exc
    except KeyError as exc:
...
async def issue_training_worker_token(
    job_id: str, http_request: Request, ttl_seconds: int = 21_600
) -> dict[str, Any]:
    try:
        job = await require_training_job_access(
            http_request, job_id, "training:jobs:create"
        )
        if not AUTH_CONFIG.jwt_secret:
            raise HTTPException(
                status_code=503,
                detail="CRAFTLY_JWT_SECRET is required for worker tokens",
            )
...
async def claim_notebook_training_job(
    ...
    try:
        ...
        if not AUTH_CONFIG.jwt_secret:
            raise HTTPException(
                status_code=503,
                detail="CRAFTLY_JWT_SECRET is required for notebook worker tokens",
            )
        token = create_jwt(
            subject=f"worker:{job.job_id}",
            secret=AUTH_CONFIG.jwt_secret,
...
async def ingest_training_events(
    job_id: str, request: TrainingEventBatch, http_request: Request
) -> dict[str, Any]:
    try:
        ...
        METRICS.inc(
            "craftly_training_events_total", value=float(len(events)), kind="batch"
        )
        if not events:
            METRICS.inc(
                "craftly_training_event_duplicates_total",
                value=float(len(request.events)),
            )
            return {
                "accepted": 0,
                "duplicates": len(request.events),
                "first_sequence": None,
                "last_sequence": None,
                "last_event_id": None,
            }
        ...
        for event in events:
            if event.loss is not None:
                METRICS.observe("craftly_training_loss", event.loss, profile=profile_id)
            if event.validation_loss is not None:
                METRICS.observe(
                    "craftly_training_validation_loss", event.validation_loss
                )
            if event.tokens_per_second is not None:
                METRICS.observe(
                    "craftly_training_tokens_per_second", event.tokens_per_second
                )
        return {
            "accepted": len(events),
...
async def list_training_audit(
    job_id: str, http_request: Request, limit: int = 100
) -> dict[str, Any]:
    try:
...
async def stream_training_events(
    job_id: str, http_request: Request, after_sequence: int = 0
) -> StreamingResponse:
    try:
        await require_training_job_access(http_request, job_id, "training:jobs:read")
    except PermissionError as exc:
    ...
    async def generate() -> Any:
        async for event in TRAINING_WORKSPACE.stream_events(
...
async def list_training_artifacts(job_id: str, http_request: Request) -> dict[str, Any]:
    try:
        await require_training_job_access(
            http_request, job_id, "training:artifacts:read"
        )
...
async def presign_training_artifact(
    job_id: str, request: ArtifactUploadRequest, http_request: Request
) -> dict[str, Any]:
    try:
        await require_training_job_access(
            http_request, job_id, "training:artifacts:write"
        )
...
async def register_training_artifact(
    ...
    try:
        ...
        artifact = await TRAINING_WORKSPACE.verify_and_register_artifact(
            job_id, request.upload, uri=request.uri
        )
...
async def commit_training_checkpoint(
    job_id: str, request: CheckpointCommitRequest, http_request: Request
) -> dict[str, Any]:
    try:
        await require_training_job_access(
            http_request, job_id, "training:artifacts:write"
        ...
        METRICS.inc("craftly_training_checkpoints_total", status="committed")
...
async def commit_training_evaluation(
    job_id: str, request: EvaluationCommitRequest, http_request: Request
) -> dict[str, Any]:
    try:
        await require_training_job_access(
            http_request, job_id, "training:artifacts:write"
        ...
        METRICS.inc("craftly_training_evaluations_total", decision=evaluation.decision)
...
async def promote_training_checkpoint(
    job_id: str, checkpoint_id: str, http_request: Request
) -> dict[str, Any]:
    try:
        ...
        METRICS.inc("craftly_training_checkpoints_total", status="promoted")
        return checkpoint.model_dump(mode="json")
    except PermissionError as exc:
        raise HTTPException(status_code=403, detail=str(exc)) from exc
    except KeyError as exc:
...
async def data_platform_status() -> dict[str, object]:
    root = Path(
        os.environ.get("CRAFTLY_DATA_PIPELINE_DIR", "artifacts/craftly/data-pipeline")
    )
...
async def enqueue_dataset_review(
    request: ReviewItemCreate, http_request: Request
) -> dict[str, Any]:
    try:
...
async def record_dataset_source_snapshot(
    request: SourceSnapshotCreate, http_request: Request
) -> dict[str, Any]:
    try:
...
async def claim_dataset_review(
    ...
    try:
        require_scope(http_request, "data:review")
        assignment = await DATASET_AUTHORITY.claim(
            record_id, request_owner(http_request), request.lease_seconds
        )
...
async def decide_dataset_review(
    ...
    try:
        require_scope(http_request, "data:review")
        item = await DATASET_AUTHORITY.decide(
            record_id, request_owner(http_request), request
        )
...
async def adjudicate_dataset_review(
    ...
    try:
        ...
        item = await DATASET_AUTHORITY.adjudicate(
            record_id, request_owner(http_request), request
        )
        return item.model_dump()
    except PermissionError as exc:
...
async def get_dataset_version(version_id: str, http_request: Request) -> dict[str, Any]:
    try:
        ...
        return ProductionDatasetManifest.model_validate_json(
...
async def promote_dataset_version(
    version_id: str, http_request: Request
) -> dict[str, Any]:
    try:
        require_scope(http_request, "data:promote")
        path = _dataset_version_manifest_path(version_id)
        policy_path = os.environ.get(
            "CRAFTLY_DATASET_TRUST_POLICY", "config/dataset_trust_policy.json"
        )
        manifest = validate_dataset_manifest_for_promotion(
            path, trust_policy_path=policy_path
        )
...
async def create_project(
    payload: ProjectCreateRequest, http_request: Request
) -> dict[str, Any]:
    try:
...
async def index_project(
    project_id: str, payload: IndexProjectRequest, http_request: Request
) -> dict[str, Any]:
    try:
    ...
    if payload.overlap_tokens >= payload.max_chunk_tokens:
        raise HTTPException(
            status_code=400,
            detail="overlap_tokens must be smaller than max_chunk_tokens",
        )
    suffixes = set(payload.include_suffixes) if payload.include_suffixes else None
    try:
        production = (
            os.environ.get("CRAFTLY_ENV", "development").lower() == "production"
        )
        if production:
...
async def get_project_index_status(
    ...
    try:
        ...
        organization_id = request_organization(http_request)
        return await asyncio.to_thread(
            rag_store(organization_id).index_status, project_id
        )
    except PermissionError as exc:
        raise HTTPException(status_code=403, detail=str(exc)) from exc
    except KeyError as exc:
...
async def get_project_index_job(
    project_id: str, job_id: str, http_request: Request
) -> dict[str, Any]:
    try:
        require_project_access(http_request, project_id, "context:read")
        organization_id = request_organization(http_request)
...
async def cancel_project_index_job(
    project_id: str, job_id: str, http_request: Request
) -> dict[str, Any]:
    try:
        ...
        active_store = rag_store(organization_id)
        job = await asyncio.to_thread(
            active_store.get_index_job, job_id, organization_id=organization_id
        )
        if job is None or str(job["project_id"]) != project_id:
            raise KeyError("index job not found")
        return await asyncio.to_thread(
...
async def create_agent_run(request: AgentRunCreateRequest) -> dict[str, Any]:
    try:
        ...
        raise HTTPException(
            status_code=404, detail="project or session not found"
        ) from exc
...
async def execute_agent_workflow(
    ...
    try:
        ...
        METRICS.inc(
            "craftly_agent_executions_total",
            status=report.status,
            applied=str(report.applied).lower(),
        )
        METRICS.observe(
            "craftly_agent_execution_duration_ms",
            report.total_duration_ms,
            status=report.status,
        )
        METRICS.observe(
            "craftly_agent_revision_attempts",
        ...
        METRICS.observe(
            "craftly_agent_gateway_duration_ms",
...
async def publish_agent_message(
    message: AgentMessage, request: Request
) -> dict[str, Any]:
    try:
...
async def list_agent_messages(
    ...
    try:
        ...
        return [
            ...
            for item in CollaborationRuntime(STORE).history(
...
async def write_agent_blackboard(
    entry: BlackboardWrite, request: Request
) -> dict[str, Any]:
    try:
        require_scope(request, "agents:blackboard:write")
        return (
            CollaborationRuntime(STORE).write_blackboard(entry).model_dump(mode="json")
        )
    except PermissionError as exc:
        raise HTTPException(status_code=403, detail=str(exc)) from exc
    except KeyError as exc:
...
async def read_agent_blackboard(
    ...
    try:
        ...
        return [
            item.model_dump(mode="json")
            for item in CollaborationRuntime(STORE).blackboard(run_id, kind=kind)
        ]
    except PermissionError as exc:
        raise HTTPException(status_code=403, detail=str(exc)) from exc
    except KeyError as exc:
...
async def project_symbols(project_id: str, request: Request) -> dict[str, Any]:
    ...
    for chunk in chunks:
        ...
        if path:
            dependencies.setdefault(path, set()).update(
                str(item) for item in dependency_values if item
            )
    ...
    return {
        ...
        "symbols": symbols,
        "dependencies": {
            path: sorted(values) for path, values in sorted(dependencies.items())
        },
    }
...
async def build_context(
    payload: ContextBuildRequest, http_request: Request
) -> dict[str, Any]:
    try:
...
async def vector_search(
    payload: VectorSearchRequest, http_request: Request, response: Response
) -> dict[str, Any]:
    try:
        project = require_project_access(
            http_request, payload.project_id, "context:read"
        )
        production = (
            os.environ.get("CRAFTLY_ENV", "development").lower() == "production"
        )
        config = (
            ...
            if production
            else VectorBackendConfig(backend=payload.backend, dims=payload.dims)
...
async def vector_sync(
    request: VectorSyncRequest, http_request: Request
) -> dict[str, Any]:
    try:
        project = require_project_access(
            http_request, request.project_id, "context:index"
        )
        production = (
            os.environ.get("CRAFTLY_ENV", "development").lower() == "production"
        )
        ...
        index = create_vector_index(
            ...
            production_vector_config()
            if production
            else VectorBackendConfig(backend=request.backend, dims=request.dims),  # type: ignore[arg-type]
        )
        report = await asyncio.to_thread(
            index.sync_project,
            organization_id=organization_id,
...
async def ingest_memory(
    request: MemoryIngestRequest, http_request: Request, project_id: str | None = None
) -> dict[str, Any]:
    try:
        require_scope(http_request, "memory:write")
        if AUTH_CONFIG.enabled and project_id is None:
            raise PermissionError(
                "authenticated memory ingestion requires a tenant-bound project_id"
            )
        ...
        report = await asyncio.to_thread(
            UnifiedMemoryManager(
                project_id=project_id, store=rag_store(organization_id)
            ).ingest,
            request,
        )
        return report.model_dump()
...
async def retrieve_memory(
    request: MemoryRetrieveRequest, http_request: Request
) -> dict[str, Any]:
    try:
        require_scope(http_request, "memory:read")
        if AUTH_CONFIG.enabled and request.project_id is None:
            raise PermissionError(
                "authenticated memory retrieval requires a tenant-bound project_id"
            )
        ...
        report = await asyncio.to_thread(
            UnifiedMemoryManager(
                project_id=request.project_id, store=rag_store(organization_id)
            ).retrieve_report,
            request.query,
            limit=request.limit,
...
async def execute_tool(
    request: ToolExecuteRequest, http_request: Request
) -> dict[str, Any]:
    try:
...
@app.post("/v1/agent/run", response_model=CraftlyRunReport)
async def run_agent(request: CraftlyRunRequest) -> CraftlyRunReport:
    return await CraftlyRuntime().run(request)
