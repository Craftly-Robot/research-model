...
def _checkpoint_integrity_gate(manifest: CheckpointManifest) -> EvalGate:
    ...
    status = (
        "pass" if not missing and not mismatched and bool(manifest.files) else "fail"
    )
    return EvalGate(
...
def _training_loss_gate(training_report: dict[str, Any] | None) -> EvalGate:
    ...
    if not losses:
        return EvalGate(
            name="training_loss",
            status="fail",
            reason="training report has no train_losses",
        )
    if not finite:
        return EvalGate(
            name="training_loss",
            status="fail",
            reason="training loss contains NaN or infinity",
        )
    first = losses[0]
    final = losses[-1]
...
def _validation_loss_gate(training_report: dict[str, Any] | None) -> EvalGate:
    if training_report is None:
        return EvalGate(
            name="validation_loss", status="warn", reason="no training report provided"
        )
    ...
    if not finite:
        return EvalGate(
            name="validation_loss",
            status="fail",
            reason="validation loss contains NaN or infinity",
        )
    first = values[0]
    final = values[-1]
...
def _checkpoint_selection_gate(
    ...
    if training_report is None:
        return EvalGate(
            ...
            status="warn",
            reason="no training report provided",
        )
...
def evaluate_checkpoint(
    *,
    ...
    benchmark_report: BenchmarkRunReport = BenchmarkHarness().run_static(
        built_in_security_tasks()
    )
    ...
    if any(
        gate.name == "checkpoint_integrity" and gate.status == "fail" for gate in gates
    ):
        recommendations.append(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate a Craftly scratch training checkpoint."
    )
    parser.add_argument("--checkpoint-manifest", required=True)
    parser.add_argument("--training-report")
    parser.add_argument("--output-dir", default="artifacts/craftly/checkpoint-eval")
